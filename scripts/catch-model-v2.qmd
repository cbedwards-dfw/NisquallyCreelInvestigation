...
dat.fit = dat |> 
  filter(life_stage == "Adult") |> 
  filter(fin_mark %in% c("UM", "AD")) |> 
  group_by(interview_id, fin_mark, event_date, year, month, fishing_duration_minutes, angler_count, angler_minutes) |>   
...
dat.sum |> 
  count(um) |> 
  rename(unmarked_encounters = um,
         interviews = n) |> 
  gt()
```

That number bounces around a lot, but in particular we see a lot more 10s that we might expect.
But maybe those 10s are from a particular time of year, or they were trips that spent a lot of time on the water (or had a lot of anglers). I'll color these trips differently to help them stand out.

```{r}
ggplot(dat.sum, aes(x = angler_minutes, y = um, col = um==10))+
...
Even though there extreme values of angler-minutes are other unmarked enocunter categories, it looks like the interviews reporting 10s probably had more average time fishing than in other categories. Here's the summary:
...
So maybe we did expect more fish caught in those interviews, on average. But not necessarily exactly 10.

What about angler count? Was it always a large group for the 10-UM encounter interviews?

```{r}
dat.sum |> 
  count(um, angler_count) |> 
  rename(unmarked_encounters = um,
         interviews = n) |> 
  gt()
```

Nope, half of them were 1 angler, at most we had 3 anglers.

Maybe it was a time of year thing?

```{r}
dat.sum |> 
  mutate(doy = yday(event_date)) |> 
  ggplot(aes(x = doy, y = um, col = um==10))+
  geom_jitter(width = 0, height = 0.3)+theme_bw()+
...
The number of anglers who reported exactly 10 unmarked encounters seems a little suspicious, but those are generally interviews that should have higher counts based on time of year and angler-minutes. This means we need to do some modeling to build a more nuanced expectation for the catches of these interviews. On the flip side, just because those reported-10 interviews look suspicious in our simple summaries doesn't mean there aren't less obvious problems hiding in the other data. We need to take a thorough look at this. 
...
Fun little bit of foreshadowing: we can look for the excess or deficit of particular values using Dharma. Here's what happens when we use the summary statistic of "Number of 10s in the data").
