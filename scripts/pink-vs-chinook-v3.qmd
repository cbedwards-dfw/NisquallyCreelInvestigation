...
Quick note from Collin: Gavin has a tutorial on getting the difference between two smooths and estimating uncertainty. Probably can adapt this to the quotient of two smooths. https://fromthebottomoftheheap.net/2017/12/14/difference-splines-ii/
...
source(here("scripts/prepare_creel_data_pinks.R"))
catch = read_csv(here("cleaned_data/key_dataframes/creel_interview_catch_withzeros.csv"))
catch.pinks = read_csv(here("cleaned_data/key_dataframes/creel_interview_catch_pinks_withzeros.csv")) |> 
  mutate(life_stage = NA)
catch.joint = rbind(catch, catch.pinks)
  
```

Quick summary of pinks:

```{r}
catch.pinks |> 
  group_by(fin_mark, year) |> 
  summarize(total = sum(fish_count)) |> 
  ungroup() |> 
  gt()
```
...
```{r}
pink.fit = catch.pinks |> 
  mutate(boat_used = as.factor(boat_used)) |> 
  group_by(interview_id, event_date, 
           year, month, fishing_duration_minutes, 
           angler_count, angler_minutes,  boat_used, fishing_start_time, fishing_end_time) |>   
...
```{r}
out.pink = gam(fish_count ~ 
                 yearfac + s(doy, by = yearfac) +
                 boat_used*angler_minutes,
          method = "REML",
          family = "nb",
          data = pink.fit |> filter(year != 2022))
